    ...
    while True:
        ...
        if random_drop == 7:
            ...
            s.sendto(feedback_struct.pack(
                *(unpacked_data[1]-1, rwnd)), client_addr)
            continue
        ...
        if rwnd > 0:
            ...
            if unpacked_data[0] == 0:
                s.sendto(feedback_struct.pack(
                    *(unpacked_data[0], rwnd)), client_addr)
                continue
...
def main():
    ...
    while True:
        ...
        my_thread = threading.Thread(
            target=server_thread, args=(client_addr, data))
        my_thread.start()
