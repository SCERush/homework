...
namespace ssl_server
{
    ...
    {
    public:
        SslConnection(int fd, const std::string &peer_ip_addr, uint16_t peer_port, SSL_CTX *ssl_ctx);
        ...
        void set_read_callback(const std::function<void(SslConnection *, Buffer *)> &callback);
        void set_want_write_callback(const std::function<void(SslConnection *, bool)> &callback);
