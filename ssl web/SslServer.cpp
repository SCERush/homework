...
namespace ssl_server
{
    ...
    {
        std::copy(buf.begin(), buf.end(), std::inserter(buffer_, buffer_.begin() + write_index_));
    ...
    static SslConnection *AcceptSocket(int serverfd, int pollfd, SSL_CTX *ssl_ctx)
    {
        ...
        int clientfd = ::accept4(serverfd, (struct sockaddr *)&addr, &addr_len, SOCK_NONBLOCK);
        if (clientfd < 0)
        {
            exit_err("accept4");
        }
        ...
        SslConnection *conn = new SslConnection(clientfd, std::string(ip_addr, strlen(ip_addr)), port, ssl_ctx);
            ...
            for (int idx = 0; idx < ret; ++idx)
            {
                ...
                if (fd == serverfd_)
                {
                    if (revents & EPOLLIN)
                    {
                        ...
                        conn->set_want_write_callback(std::bind(&SslServer::PollOnWrite, this, _1, _2));
                        conn->set_close_callback(std::bind(&SslServer::CloseConnection, this, _1));
                ...
                else
                {
                    ...
                    SslConnection *conn = reinterpret_cast<SslConnection *>(evts_[idx].data.ptr);
                    if (conn)
                    {
    ...
    void SslServer::set_message_callback(const std::function<void(SslConnection *, Buffer *)> &callback)
    {
    ...
    SslConnection::SslConnection(int fd, const std::string &peer_ip_addr, uint16_t peer_port, SSL_CTX *ssl_ctx)
        : clientfd_(fd),
    ...
    void SslConnection::Encrypt()
    {
        ...
        if (want_write_callback_)
            want_write_callback_(this, true);
    }
    ...
    const std::string &SslConnection::peer_ip() const
    {
        return peer_ip_;
    }

    uint16_t SslConnection::peer_port() const
    {
        return peer_port_;
    }

    void SslConnection::set_close_callback(const std::function<void(int)> &callback)
    {
        close_callback_ = callback;
    }

    void SslConnection::set_read_callback(const std::function<void(SslConnection *, Buffer *)> &callback)
    {
        read_callback_ = callback;
    }

    void SslConnection::set_want_write_callback(const std::function<void(SslConnection *, bool)> &callback)
    {
    ...
    bool SslConnection::Send()
    {
        ...
        if (n >= 0 && want_write_callback_)
            want_write_callback_(this, false);
