 ...
 * port: 端口号
 * maxListen: 最大监听数目
 * 返回值: 监听Socket结构。套接字创建失败：Socket.sockId=-1;
 * 端口绑定失败：Socket.sockId=-2; 设置监听失败：Socket.sockId=-2
 ********************************************************************************/
...
/********************************************************************************
 * 功能：基于TCP在chunked模式下，从Socket缓冲区读取一个块的有效内容，
 * 为malloc分配的内存，应!!显式释放！！尾部额外添加'\0'，但不保证有用
 * sock: 客户端Socket
 * dataLength: 返回有效数据长度
