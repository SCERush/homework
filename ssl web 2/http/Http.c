...
int DealWithRequest(Socket sock) //利用libevent并发处理请求
{
    // libevent并发
...
void accept_cb(int fd, short events, void *arg) //若event正常则接受请求并输出请求信息
{
    Eventarg *eventarg = (Eventarg *)arg;
    if (events == EV_TIMEOUT)
    {
        if (eventarg->firsttime == true)
            return;
        else
            CloseHttp(eventarg);
    }
    else
    {
        ...
        if (memcmp(req->contentLength, "\0", 1) != 0)
        {
            ...
            RecvDataByLength(*(eventarg->sock), req->bodyptr, len, 0); //接收数据
        }
        if (memcmp(req->Encoding, "chunked", 7) == 0)
            req->bodyptr = (char *)(eventarg->sock);
        printRequest(req); //输出requeset信息

        struct Block *block = init(req);

        printResponse(block);                   //输出response信息
        sendResponse(block, *(eventarg->sock)); //发送回应
...
{ //释放Block所占空间
    ...
    if (block->response != NULL)
        freeResponse(block->response);
    if (block->fileinfo != NULL)
        freeFileinfo(block->fileinfo);
    free(block);
}
...
int ReadHttpHeaderBytes(Socket sock, char *data, int maxBuffer) //读取http头信息
{
...
int ResponseHttpWithHtml(Socket sock, HttpKind httpKind, char *htmlPath) //发送客户端请求的html网页
{
    // 计算http头部内容
...
int ResponseHttp(Socket sock, char *contentType, char *filePath, Bool allowChunked) //返回http回应
{
    // 计算获取正确的文件path
    char path[500];
    memset(path, 0, 500);
...
void CloseHttp(Eventarg *eventarg) //关闭http连接
{
    ...
    free(eventarg);
    printf("[Info] HTTPS connection close.\n");
    return;
}