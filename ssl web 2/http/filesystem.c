...
int createfile(char *name, struct Request *req) //创建文件并写入信息
{
    struct FileInfo *rst;
    FILE *fp;
    int len;
...
struct FileInfo *file2body(char *path) //返回文件信息
{
    //错误则返回NULL，可能需要响应错误码
    //记得free
    struct FileInfo *rst;
...
int GetFileLength(char *path) //返回文件长度
{
...
int EndWithString(char *str1, char *str2) //判断str2是否是str1的末尾子串
{
...
void GetFileName(char *path, char *result) //获得文件名
{
...
void getType(char *file_name, char *extension) //获得文件扩展名即文件类型
{
